...
import { resumeV6PlanBuild, startV6PlanBuild, type V6StreamCallbacks } from '../../src/lib/client/v6-plan-client'
import type { ClarificationQuestion, ClarificationRound } from '../../src/lib/pipeline/v6/types'
import styles from './V6PlanFlow.module.css'

type FlowStep = 'input' | 'processing' | 'clarifying' | 'completed'

interface V6PlanFlowProps {
  profileId: string
  provider: string
}
...
export function V6PlanFlow({ profileId, provider }: V6PlanFlowProps) {
  ...
  function createCallbacks(origin: 'start' | 'resume', runId: number): V6StreamCallbacks {
  ...
  async function handleStart(): Promise<void> {
    ...
    await startV6PlanBuild(normalizedGoal, profileId, provider, createCallbacks('start', runId))
  ...
  async function handleResume(): Promise<void> {
    ...
    await resumeV6PlanBuild(sessionId, answers, createCallbacks('resume', runId))
