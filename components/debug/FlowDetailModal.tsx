...
function IntakeDetail({ data }: { data: Record<string, unknown> }) {
  const d = data as { profileId?: string }
  return (
    <>
    ...
    <>
      ...
      {d.summary && (
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.25rem' }}>
          {d.summary.pass > 0 && <span style={{ fontSize: '0.8rem', color: '#6ed7a5' }}>✓ {d.summary.pass} pass</span>}
          {d.summary.warn > 0 && <span style={{ fontSize: '0.8rem', color: '#f2bf82' }}>⚠ {d.summary.warn} warn</span>}
          {d.summary.fail > 0 && <span style={{ fontSize: '0.8rem', color: '#ff6b6b' }}>✗ {d.summary.fail} fail</span>}
        </div>
      )}
          ...
          {history.map((h, i) => (
            <div key={i} className="repair-attempt-row">
              <div className="repair-attempt-header">
                ...
                <span className="repair-attempt-score">score: {h.qualityScore}/100 · {t('debug.flow.repair_findings_count', { count: h.findingsCount })}</span>
              </div>
              {h.repairNotes && (
...
function OutputDetail({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <>
      ...
      <div className={modeClass}>{mode}</div>
