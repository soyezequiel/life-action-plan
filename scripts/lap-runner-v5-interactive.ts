...
import { collectInteractiveInput, type InteractiveRunnerInput } from './interactive-prompts';
import { renderDiagnosticReport, type RenderMode } from './diagnostic-renderer';

const DEFAULT_OUTPUT_FILE = resolve(process.cwd(), 'tmp/pipeline-v5-interactive.json');
const SHOWCASE_CALENDAR_WEEKS = 20;
const WEEK_DAYS = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as const;
const WORK_DAYS = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday'] as const;
...
function makeAvailability(startTime: string, endTime: string): AvailabilityWindow[] {
  return WEEK_DAYS.map((day) => ({ day, startTime, endTime }));
}

function makeBlocked(input: InteractiveRunnerInput): BlockedSlot[] {
  const workBlocked = input.worksMondayToFriday && input.workStartTime && input.workEndTime
    ? WORK_DAYS.map((day) => ({
      day,
      startTime: input.workStartTime ?? '09:00',
      endTime: input.workEndTime ?? '18:00',
      reason: 'Trabajo',
    }))
    : [];
  ...
  return [
    ...workBlocked,
    ...input.extraBlockedSlots.map((slot) => ({
      day: slot.day,
...
function createHabitStateStore(initialState: HabitState | null): HabitStateStore {
  let savedStates = initialState ? [initialState] : [];

  return {
    async loadByProgressionKeys(progressionKeys: string[]) {
      console.error(`[V5 Interactive] HabitState.loadByProgressionKeys -> ${progressionKeys.join(', ') || '(none)'}`);
      return savedStates.filter((state) => progressionKeys.includes(state.progressionKey));
    },
    async save(states: HabitState[]) {
      savedStates = states;
      console.error(`[V5 Interactive] HabitState.save -> ${savedStates.map((state) => state.progressionKey).join(', ') || '(none)'}`);
    }
  };
}
...
async function printCodexUsageSnapshot(
  stage: 'antes' | 'despues',
  timezone: string,
): Promise<import('../src/lib/auth/codex-usage').CodexUsageSnapshot | null> {
  try {
    const snapshot = await fetchCodexUsageSnapshot();
    console.error(`[V5 Interactive] ${formatCodexCreditsLine(snapshot)} (${stage})`);
    for (const line of formatCodexRateLimitLines(snapshot, timezone)) {
      console.error(`[V5 Interactive] ${line} (${stage})`);
    }
    return snapshot;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error(`[V5 Interactive] No pude leer el credito de Codex (${stage}): ${message}`);
    return null;
  }
}
...
function openUrlInBrowser(url: string): void {
  ...
  child.on('error', (error) => {
    console.error(`[V5 Interactive] No pude abrir el navegador para ${url}: ${error.message}`);
  });
  child.unref();
}

async function maybeOpenPage(url: string, label: string): Promise<void> {
  if (!(await canReachPage(url))) {
    console.error(`[V5 Interactive] No abri ${label} porque la app no responde en ${url}.`);
    return;
  }

  openUrlInBrowser(url);
  console.error(`[V5 Interactive] Abriendo ${label}: ${url}`);
}
...
function resolveWeekStart(weekStartDate: string, timezone: string): DateTime {
  const parsed = DateTime.fromISO(weekStartDate, { setZone: true });
  if (parsed.isValid) {
    return parsed.setZone(timezone).startOf('day');
  }
...
function buildShowcaseDetail(
  ...
  const weekStart = resolveWeekStart(weekStartDate, timezone);
...
function buildShowcasePackage(
  ...
  const detail = buildShowcaseDetail(pkg, horizonWeeks, options.weekStartDate, options.timezone);
  const frequencies = buildShowcaseFrequencies(detail.weeks[0]?.scheduledEvents ?? []);
  const skeleton = buildShowcaseSkeleton(
    pkg,
    strategy,
    frequencies,
    horizonWeeks,
...
async function run(): Promise<void> {
  ...
  const interactiveInput = await collectInteractiveInput();
  ...
  const traceId = traceCollector.startTrace('cli-v5-interactive', modelId, {
    command: `npx tsx scripts/lap-runner-v5-interactive.ts ${process.argv.slice(2).join(' ')}`.trim()
  });
  const runtimeRecorder = createPipelineRuntimeRecorder({
    source: 'cli-v5',
    modelId,
    goalText: interactiveInput.goalText,
    outputFile
  });
  let runError: Error | null = null;
  const schedulingContext = buildSchedulingContextFromRunnerConfig({
    timezone: interactiveInput.timezone,
    availability: makeAvailability(interactiveInput.wakeTime, interactiveInput.sleepTime),
    blocked: makeBlocked(interactiveInput),
  });
  const runner = new FlowRunnerV5({
    runtime,
    text: interactiveInput.goalText,
    answers: interactiveInput.answers,
    ...
    goalId: interactiveInput.goalId,
    domainHint: interactiveInput.domainHint,
    slackPolicy: interactiveInput.slackPolicy,
    habitStateStore: createHabitStateStore(interactiveInput.initialHabitState),
    previousProgressionKeys: interactiveInput.previousProgressionKeys,
    inlineAdaptive
  });
  ...
  const handleSigint = () => {
    console.error('\n[V5 Interactive] Interrumpido por el usuario.');
    process.exit(130);
  };
  process.on('SIGINT', handleSigint);

  console.error(`[V5 Interactive] Modelo activo: ${modelId}`);
  console.error(`[V5 Interactive] Autenticacion: ${authLabel}`);
  if (codexIdentity) {
    ...
    console.error(`[V5 Interactive] Cuenta Codex: ${formatCodexAccountLabel(codexIdentity)}`);
    console.error(`[V5 Interactive] Account ID: ${codexIdentity.accountId}${planLabel}`);
    console.error(`[V5 Interactive] Fuente auth: ${authSourceLabel} (${codexIdentity.authFilePath})`);
    codexUsageBefore = await printCodexUsageSnapshot('antes', schedulingContext.timezone);
  }
  console.error(`[V5 Interactive] Running ${modelId} -> ${outputFile}`);
  if (autoOpenBrowser) {
    await maybeOpenPage(viewerLinks.flowUrl, 'viewer');
  }
  ...
  const tracker: FlowRunnerV5Tracker = {
    onPhaseStart: (phase, details) => {
      ...
      console.error(`[V5 Interactive] -> ${phase}`);
    },
    ...
    onRepairAttempt: (attempt, maxAttempts, findings) => {
      ...
      console.error(`[V5 Interactive] repair ${attempt}/${maxAttempts} with ${findings.length} findings`);
    },
  ...
  try {
    ...
    if (!context.package) {
      throw new Error('V5 interactive runner finished without package output.');
    }
    ...
    if (autoOpenBrowser) {
      await maybeOpenPage(buildDashboardLaunchUrl(viewerLinks.dashboardUrl), 'resultados');
    }
    ...
    if (diagnosticMode.enabled) {
      ...
      console.log('\n--- V5 INTERACTIVE PIPELINE RESULT START ---');
      ...
      console.log('--- V5 INTERACTIVE PIPELINE RESULT END ---\n');
    }

    console.error('[V5 Interactive] Vistas disponibles:');
    console.error(`[V5 Interactive] Flow Viewer: ${viewerLinks.flowUrl}`);
    console.error(`[V5 Interactive] Dashboard V5: ${viewerLinks.dashboardUrl}`);
    console.error(`[V5 Interactive] JSON: ${outputFile}`);
    console.error('[V5 Interactive] Si no ves las vistas, levanta la app con npm run dev y abre esas URLs.');
  } catch (error) {
    ...
    if (traceUsage.spans > 0) {
      ...
      console.error(
        `[V5 Interactive] Tokens consumidos: entrada=${traceUsage.promptTokens} | salida=${traceUsage.completionTokens} | llamadas=${traceUsage.spans}`
      );
    }

    if (codexIdentity) {
      const codexUsageAfter = await printCodexUsageSnapshot('despues', schedulingContext.timezone);
      for (const deltaLine of formatCodexUsageDeltaLines(codexUsageBefore, codexUsageAfter)) {
        console.error(`[V5 Interactive] ${deltaLine}`);
      }
    }
...
run().catch((error) => {
  console.error('[V5 Interactive] Runtime error:', error);
  process.exit(1);
});
